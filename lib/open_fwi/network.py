...
def replace_legacy(old_dict):
    ...
    for k, v in old_dict.items():
        k = k.replace("Conv2DwithBN", "layers").replace("Conv2DwithBN_Tanh", "layers").replace("Deconv2DwithBN", "layers").replace("ResizeConv2DwithBN", "layers")
        li.append((k, v))
    return OrderedDict(li)


class Conv2DwithBN(nn.Module):
    def __init__(self, in_fea, out_fea, kernel_size=3, stride=1, padding=1, bn=True, relu_slop=0.2, dropout=None):
...
model_dict = {"InversionNet": InversionNet, "Discriminator": Discriminator, "UPFWI": FCN4_Deep_Resize_2}
