...
def train_one_epoch(model, criterion, optimizer, lr_scheduler, dataloader, device, epoch, print_freq, writer):
    ...
    for data, label in metric_logger.log_every(dataloader, print_freq, header):
        ...
        metric_logger.update(loss=loss_val, loss_g1v=loss_g1v_val, loss_g2v=loss_g2v_val, lr=optimizer.param_groups[0]["lr"])
...
def main(args):
    ...
    transform_data = Compose([T.LogTransform(k=args.k), T.MinMaxNormalize(T.log_transform(ctx["data_min"], k=args.k), T.log_transform(ctx["data_max"], k=args.k))])
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        checkpoint = {"model": model_without_ddp.state_dict(), "optimizer": optimizer.state_dict(), "lr_scheduler": lr_scheduler.state_dict(), "epoch": epoch, "step": step, "args": args}
