...
def train_one_epoch(model, model_d, criterion_g, criterion_d, optimizer_g, optimizer_d, lr_schedulers, dataloader, device, epoch, print_freq, writer, n_critic=5):
    global step
    model.train()
    model_d.train()
    ...
    for data, label in metric_logger.log_every(dataloader, print_freq, header):
        ...
        metric_logger.update(lr_g=optimizer_g.param_groups[0]["lr"], lr_d=optimizer_d.param_groups[0]["lr"])
...
def evaluate(model, criterion, dataloader, device, writer):
    ...
    with torch.no_grad():
        for data, label in metric_logger.log_every(dataloader, 20, header):
            ...
            metric_logger.update(loss=loss.item(), loss_g1v=loss_g1v.item(), loss_g2v=loss_g2v.item())
...
def main(args):
    ...
    transform_data = Compose([T.LogTransform(k=args.k), T.MinMaxNormalize(log_data_min, log_data_max)])
    transform_label = Compose([T.MinMaxNormalize(ctx["label_min"], ctx["label_max"])])
    if args.train_anno[-3:] == "txt":
        dataset_train = FWIDataset(args.train_anno, preload=True, sample_ratio=args.sample_temporal, file_size=ctx["file_size"], transform_data=transform_data, transform_label=transform_label)
    else:
        dataset_train = torch.load(args.train_anno)

    print("Loading validation data")
    if args.val_anno[-3:] == "txt":
        dataset_valid = FWIDataset(args.val_anno, preload=True, sample_ratio=args.sample_temporal, file_size=ctx["file_size"], transform_data=transform_data, transform_label=transform_label)
    else:
    ...
    dataloader_train = DataLoader(dataset_train, batch_size=args.batch_size, sampler=train_sampler, num_workers=args.workers, pin_memory=True, drop_last=True, collate_fn=default_collate)

    dataloader_valid = DataLoader(dataset_valid, batch_size=args.batch_size, sampler=valid_sampler, num_workers=args.workers, pin_memory=True, collate_fn=default_collate)
    ...
    model = network.model_dict[args.model](upsample_mode=args.up_mode, sample_spatial=args.sample_spatial, sample_temporal=args.sample_temporal).to(device)
    ...
    lr_schedulers = [WarmupMultiStepLR(optimizer, milestones=lr_milestones, gamma=args.lr_gamma, warmup_iters=warmup_iters, warmup_factor=1e-5) for optimizer in [optimizer_g, optimizer_d]]
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_one_epoch(model, model_d, criterion_g, criterion_d, optimizer_g, optimizer_d, lr_schedulers, dataloader_train, device, epoch, args.print_freq, train_writer, args.n_critic)
