...
def evaluate(model, criterions, dataloader, device, k, ctx, vis_path, vis_batch, vis_sample, missing, std):
    model.eval()
                ...
                for i in range(vis_sample):
                    ...
                    if missing or std:
                        for ch in [2]:  # range(data.shape[1]):
                            plot_seismic(data_np[i, ch], data_noise_np[i, ch], f"{vis_path}/S_{batch_idx}_{i}_{ch}.png", vmin=ctx["data_min"] * 0.01, vmax=ctx["data_max"] * 0.01)
            batch_idx += 1
...
def main(args):
    ...
    transform_valid_label = Compose([T.MinMaxNormalize(ctx["label_min"], ctx["label_max"])])
    if args.val_anno[-3:] == "txt":
        dataset_valid = FWIDataset(args.val_anno, sample_ratio=args.sample_temporal, file_size=ctx["file_size"], transform_data=transform_valid_data, transform_label=transform_valid_label)
    else:
        dataset_valid = torch.load(args.val_anno)

    print("Creating data loaders")
    valid_sampler = SequentialSampler(dataset_valid)
    dataloader_valid = torch.utils.data.DataLoader(dataset_valid, batch_size=args.batch_size, sampler=valid_sampler, num_workers=args.workers, pin_memory=True, collate_fn=default_collate)
    ...
    model = network.model_dict[args.model](upsample_mode=args.up_mode, sample_spatial=args.sample_spatial, sample_temporal=args.sample_temporal, norm=args.norm).to(device)

    criterions = {"MAE": lambda x, y: np.mean(np.abs(x - y)), "MSE": lambda x, y: np.mean((x - y) ** 2)}
    ...
    evaluate(model, criterions, dataloader_valid, device, args.k, ctx, vis_path, args.vis_batch, args.vis_sample, args.missing, args.std)
