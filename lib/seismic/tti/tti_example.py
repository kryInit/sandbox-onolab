...
def tti_setup(shape=(50, 50, 50), spacing=(20.0, 20.0, 20.0), tn=250.0, kernel="centered", space_order=4, nbl=10, preset="layers-tti", **kwargs):

    # Two layer model for true velocity
    model = demo_model(preset, shape=shape, spacing=spacing, space_order=space_order, nbl=nbl, **kwargs)

    # Source and receiver geometries
    geometry = setup_geometry(model, tn)

    return AnisotropicWaveSolver(model, geometry, space_order=space_order, kernel=kernel, **kwargs)


def run(shape=(50, 50, 50), spacing=(20.0, 20.0, 20.0), tn=250.0, autotune=False, space_order=4, nbl=10, preset="layers-tti", kernel="centered", full_run=False, checkpointing=False, **kwargs):

    solver = tti_setup(shape=shape, spacing=spacing, tn=tn, space_order=space_order, nbl=nbl, kernel=kernel, preset=preset, **kwargs)
    ...
    if preset in ["constant-tti", "constant-tti-noazimuth"]:
        ...
        if len(shape) > 2 and preset not in ["constant-tti-noazimuth"]:
            phi = 0.4
        solver.forward(save=save, vp=vp, epsilon=epsilon, delta=delta, theta=theta, phi=phi)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--noazimuth", dest="azi", default=False, action="store_true", help="Whether or not to use an azimuth angle")
    parser.add_argument(
        "-k",
        dest="kernel",
        default="centered",
