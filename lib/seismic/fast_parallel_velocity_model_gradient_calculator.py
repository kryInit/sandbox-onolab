...
def calc_grad_helper(
    ...
    new_current_model = SeismicModel(space_order=2, vp=current_velocity_model.T, origin=(0, 0), shape=current_model.shape,
                                      dtype=np.float32, spacing=current_model.spacing, nbl=current_model.nbl, bcs="damp",
                                      fs=False)
    ...
    # 観測データと計算データの残差を計算
    residual = calculated_waveform.data - observed_waveform

    # 雑なobjective計算
    objective = 0.5 * np.sum(np.abs(residual ** 2))
