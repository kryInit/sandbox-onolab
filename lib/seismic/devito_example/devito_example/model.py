import numpy as np
from devito import Abs, Constant, Eq, Function, Grid, Inc, Operator, SubDimension, SubDomain, div, sin, warning
...
__all__ = ["SeismicModel", "Model", "ModelElastic", "ModelViscoelastic", "ModelViscoacoustic"]
...
class GenericModel(object):
    ...
    def __init__(
        ...
        if grid is None:
            ...
            self.grid = Grid(extent=extent, shape=shape_pml, origin=origin_pml, dtype=dtype, subdomains=subdomains)
        else:
            self.grid = grid
    ...
    def _gen_phys_param(self, field, name, space_order, is_param=True, default_value=0, **kwargs):
        if field is None:
            return default_value
        if isinstance(field, np.ndarray):
            function = Function(name=name, grid=self.grid, space_order=space_order, parameter=is_param)
...
class SeismicModel(GenericModel):
    """
    The physical model used in devito_example inversion processes.
    ...
    _known_parameters = ["vp", "damp", "vs", "b", "epsilon", "delta", "theta", "phi", "qp", "qs", "lam", "mu"]

    def __init__(self, origin, spacing, shape, space_order, vp, nbl=20, fs=False, dtype=np.float32, subdomains=(), bcs="mask", grid=None, **kwargs):
        super().__init__(origin, spacing, shape, space_order, nbl, dtype, subdomains, grid=grid, bcs=bcs, fs=fs)
    ...
    def _initialize_physics(self, vp, space_order, **kwargs):
        ...
        if "vs" in kwargs:
            ...
            # All other devito_example models have at least a velocity
    ...
    def update(self, name, value):
        ...
        if isinstance(value, np.ndarray):
            if value.shape == param.shape:
                ...
                raise ValueError("Incorrect input size %s for model" % value.shape + " %s without or %s with padding" % (self.shape, param.shape))
        else:
            param.data = value
