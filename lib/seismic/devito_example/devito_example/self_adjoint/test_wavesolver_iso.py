...
from examples.seismic.self_adjoint import SaIsoAcousticWaveSolver, acoustic_sa_setup, setup_w_over_q
...
class TestWavesolver(object):
    ...
    def test_linearity_forward_F(self, shape, dtype, so):
        ...
        info("linearity forward F %s (so=%d) rms 1,2,diff; " "%+16.10e %+16.10e %+16.10e" % (shape, so, np.sqrt(np.mean(rec1.data**2)), np.sqrt(np.mean(rec2.data**2)), np.sqrt(np.mean(diff**2))))
        tol = 1.0e-12
    ...
    def test_linearity_adjoint_F(self, shape, dtype, so):
        ...
        info("linearity adjoint F %s (so=%d) rms 1,2,diff; " "%+16.10e %+16.10e %+16.10e" % (shape, so, np.sqrt(np.mean(src1.data**2)), np.sqrt(np.mean(src2.data**2)), np.sqrt(np.mean(diff**2))))
        tol = 1.0e-12
        assert np.allclose(diff, 0.0, atol=tol)
    ...
    def test_linearization_F(self, shape, dtype, so):
        ...
        if len(shape) == 2:
            ...
            nx, ny, nz = shape
            dm.data[nx2 - size : nx2 + size, ny2 - size : ny2 + size, nz2 - size : nz2 + size] = -1 + 2 * np.random.rand(ns, ns, ns)
    ...
    def test_linearity_forward_J(self, shape, dtype, so):
        ...
        if len(shape) == 2:
            ...
            nx2, ny2, nz2 = shape[0] // 2, shape[1] // 2, shape[2] // 2
            nx, ny, nz = shape
            m1.data[nx2 - size : nx2 + size, ny2 - size : ny2 + size, nz2 - size : nz2 + size] = -1 + 2 * np.random.rand(ns, ns, ns)
        ...
        rms2 = np.sqrt(np.mean(rec2.data**2))
        diff = (rec1.data - rec2.data) / rms2
        info("linearity forward J %s (so=%d) rms 1,2,diff; " "%+16.10e %+16.10e %+16.10e" % (shape, so, np.sqrt(np.mean(rec1.data**2)), np.sqrt(np.mean(rec2.data**2)), np.sqrt(np.mean(diff**2))))
        tol = 1.0e-12
        assert np.allclose(diff, 0.0, atol=tol)
    ...
    def test_linearity_adjoint_J(self, shape, dtype, so):
        ...
        info("linearity adjoint J %s (so=%d) rms 1,2,diff; " "%+16.10e %+16.10e %+16.10e" % (shape, so, np.sqrt(np.mean(dm1.data**2)), np.sqrt(np.mean(dm2.data**2)), np.sqrt(np.mean(diff**2))))
    ...
    def test_adjoint_J(self, shape, dtype, so):
        ...
        if len(shape) == 2:
            ...
            nx2, ny2, nz2 = shape[0] // 2, shape[1] // 2, shape[2] // 2
            nx, ny, nz = shape
            dm1.data[nx2 - size : nx2 + size, ny2 - size : ny2 + size, nz2 - size : nz2 + size] = -1 + 2 * np.random.rand(ns, ns, ns)
    ...
    def test_analytic_comparison_2d(self, dtype, so):
        ...
        model = Model(origin=o, shape=shape, vp=v0, b=1.0, spacing=spacing, nbl=npad, space_order=space_order, bcs=init_damp)
        ...
        def analytic_response():
            ...
            examples/devito_example/acoustic/accuracy.ipynb
            ...
            srcpad = RickerSource(name="srcpad", grid=model.grid, f0=fpeak, npoint=1, time_range=time_axis_pad, t0w=t0w)
            nf = int(ntpad / 2 + 1)
            df = 1.0 / tmaxpad
