...
def seismic_args(description):
    """
    Command line options for the devito_example examples
    """
        ...
        def __call__(self, parser, args, values, option_string=None):
            try:
                ...
                if not isinstance(values, tuple) and len(values) >= 1:
                    raise ArgumentError(self, ("Invalid choice `%s` (`opt` must be " "either str or tuple)" % str(values)))
                opt = values[0]
            except NameError:
                # E.g. `'advanced'`
                opt = values
            if opt not in configuration._accepted["opt"]:
                raise ArgumentError(self, ("Invalid choice `%s` (choose from %s)" % (opt, str(configuration._accepted["opt"]))))
            setattr(args, self.dest, values)

    parser = ArgumentParser(description=description)
    parser.add_argument("-nd", dest="ndim", default=3, type=int, help="Number of dimensions")
    parser.add_argument("-d", "--shape", default=(51, 51, 51), type=int, nargs="+", help="Number of grid points along each axis")
    parser.add_argument("-f", "--full", default=False, action="store_true", help="Execute all operators and store forward wavefield")
    parser.add_argument("-so", "--space_order", default=4, type=int, help="Space order of the simulation")
    parser.add_argument("--nbl", default=40, type=int, help="Number of boundary layers around the domain")
    parser.add_argument("--constant", default=False, action="store_true", help="Constant velocity model, default is a two layer model")
    parser.add_argument("--checkpointing", default=False, action="store_true", help="Use checkpointing, default is False")
    parser.add_argument("-opt", default="advanced", action=_opt_action, help="Performance optimization level")
    parser.add_argument(
        "-a",
        "--autotune",
        default="off",
    ...
    parser.add_argument("-dtype", action=_dtype_store, dest="dtype", default=np.float32, choices=["float32", "float64"])
