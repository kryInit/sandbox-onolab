from devito import NODE, Eq, Function, Inc, Operator, TimeFunction, cos, div, grad, sin, solve, sqrt
...
def Gzz_centered(model, field):
    ...
    Gz = sintheta * cosphi * field.dx(fd_order=order1, x0=x + dx) + sintheta * sinphi * field.dy(fd_order=order1, x0=y + dy) + costheta * field.dz(fd_order=order1, x0=z + dz)
...
def Gzz_centered_2d(model, field):
    ...
    Gz = sintheta * field.dx(fd_order=order1, x0=x + dx) + costheta * field.dy(fd_order=order1, x0=y + dy)
...
def Gh_centered(model, field):
    ...
    if b is not None:
        ...
        lap = div(b * grad(field, shift=0.5, order=so), shift=-0.5, order=so)
    else:
        lap = field.laplace
    return lap - Gzz
...
def particle_velocity_fields(model, space_order):
    ...
    if model.grid.dim == 2:
        ...
        vx = TimeFunction(name="vx", grid=model.grid, staggered=stagg_x, time_order=1, space_order=space_order)
        vz = TimeFunction(name="vz", grid=model.grid, staggered=stagg_z, time_order=1, space_order=space_order)
        vy = None
        ...
        # Create symbols for forward wavefield, source and receivers
        vx = TimeFunction(name="vx", grid=model.grid, staggered=stagg_x, time_order=1, space_order=space_order)
        vy = TimeFunction(name="vy", grid=model.grid, staggered=stagg_y, time_order=1, space_order=space_order)
        vz = TimeFunction(name="vz", grid=model.grid, staggered=stagg_z, time_order=1, space_order=space_order)
...
def kernel_staggered_2d(model, u, v, **kwargs):
    ...
    if forward:
        ...
        ph_eq = Eq(u.forward, dampl * (u - s / m * (epsilon * dvx + delta * dvz)) + s / m * qu)
    else:
        # Stencils
        phdx = (costheta * epsilon * u).dx - (sintheta * epsilon * u).dyc + (costheta * delta * v).dx - (sintheta * delta * v).dyc
        u_vx = Eq(vx.backward, dampl * vx + dampl * s * phdx)

        pvdz = (sintheta * delta * u).dxc + (costheta * delta * u).dy + (sintheta * v).dxc + (costheta * v).dy
...
def kernel_staggered_3d(model, u, v, **kwargs):
    ...
    if forward:
        # Stencils
        phdx = costheta * cosphi * u.dx + costheta * sinphi * u.dyc - sintheta * u.dzc
        u_vx = Eq(vx.forward, dampl * vx - dampl * s * phdx)

        phdy = -sinphi * u.dxc + cosphi * u.dy
        u_vy = Eq(vy.forward, dampl * vy - dampl * s * phdy)

        pvdz = sintheta * cosphi * v.dxc + sintheta * sinphi * v.dyc + costheta * v.dz
        u_vz = Eq(vz.forward, dampl * vz - dampl * s * pvdz)

        dvx = costheta * cosphi * vx.forward.dx + costheta * sinphi * vx.forward.dyc - sintheta * vx.forward.dzc
        dvy = -sinphi * vy.forward.dxc + cosphi * vy.forward.dy
        dvz = sintheta * cosphi * vz.forward.dxc + sintheta * sinphi * vz.forward.dyc + costheta * vz.forward.dz
        # u and v equations
        pv_eq = Eq(v.forward, dampl * (v - s / m * (delta * (dvx + dvy) + dvz)) + s / m * qv)

        ph_eq = Eq(u.forward, dampl * (u - s / m * (epsilon * (dvx + dvy) + delta * dvz)) + s / m * qu)
    else:
        # Stencils
        phdx = (
            (costheta * cosphi * epsilon * u).dx
            + (costheta * sinphi * epsilon * u).dyc
            - (sintheta * epsilon * u).dzc
            + (costheta * cosphi * delta * v).dx
            + (costheta * sinphi * delta * v).dyc
            - (sintheta * delta * v).dzc
        )
        u_vx = Eq(vx.backward, dampl * vx + dampl * s * phdx)

        phdy = -(sinphi * epsilon * u).dxc + (cosphi * epsilon * u).dy - (sinphi * delta * v).dxc + (cosphi * delta * v).dy
        u_vy = Eq(vy.backward, dampl * vy + dampl * s * phdy)

        pvdz = (sintheta * cosphi * delta * u).dxc + (sintheta * sinphi * delta * u).dyc + (costheta * delta * u).dz + (sintheta * cosphi * v).dxc + (sintheta * sinphi * v).dyc + (costheta * v).dz
        u_vz = Eq(vz.backward, dampl * vz + dampl * s * pvdz)

        dvx = (costheta * cosphi * vx.backward).dx + (costheta * sinphi * vx.backward).dyc - (sintheta * vx.backward).dzc
        dvy = (-sinphi * vy.backward).dxc + (cosphi * vy.backward).dy
        dvz = (sintheta * cosphi * vz.backward).dxc + (sintheta * sinphi * vz.backward).dyc + (costheta * vz.backward).dz
        # u and v equations
        pv_eq = Eq(v.backward, dampl * (v + s / m * dvz))
...
def ForwardOperator(model, geometry, space_order=4, save=False, kernel="centered", **kwargs):
    """
...
def AdjointOperator(model, geometry, space_order=4, kernel="centered", **kwargs):
    """
    ...
    # Create symbols for forward wavefield, source and receivers
    p = TimeFunction(name="p", grid=model.grid, staggered=stagg_p, time_order=time_order, space_order=space_order)
    r = TimeFunction(name="r", grid=model.grid, staggered=stagg_r, time_order=time_order, space_order=space_order)
...
def JacobianOperator(model, geometry, space_order=4, **kwargs):
    ...
    u0 = TimeFunction(name="u0", grid=model.grid, save=None, time_order=time_order, space_order=space_order)
    v0 = TimeFunction(name="v0", grid=model.grid, save=None, time_order=time_order, space_order=space_order)
    du = TimeFunction(name="du", grid=model.grid, save=None, time_order=2, space_order=space_order)
    dv = TimeFunction(name="dv", grid=model.grid, save=None, time_order=2, space_order=space_order)
    ...
    # Substitute spacing terms to reduce flops
    return Operator(eqn1 + src_term + eqn2 + rec_term, subs=model.spacing_map, name="BornTTI", **kwargs)
...
def JacobianAdjOperator(model, geometry, space_order=4, save=True, **kwargs):
    ...
    u0 = TimeFunction(name="u0", grid=model.grid, save=geometry.nt if save else None, time_order=time_order, space_order=space_order)
    v0 = TimeFunction(name="v0", grid=model.grid, save=geometry.nt if save else None, time_order=time_order, space_order=space_order)

    du = TimeFunction(name="du", grid=model.grid, save=None, time_order=time_order, space_order=space_order)
    dv = TimeFunction(name="dv", grid=model.grid, save=None, time_order=time_order, space_order=space_order)
    ...
    # Substitute spacing terms to reduce flops
    return Operator(eqn + rec_term + [dm_update], subs=model.spacing_map, name="GradientTTI", **kwargs)


kernels = {
    ("centered", 3): kernel_centered,
    ("centered", 2): kernel_centered,
    ("staggered", 3): kernel_staggered_3d,
    ("staggered", 2): kernel_staggered_2d,
}
