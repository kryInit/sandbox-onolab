# coding: utf-8
from devito import NODE, CheckpointOperator, DevitoCheckpoint, Function, Revolver, TimeFunction, warning
from devito.tools import memoized_meth
from examples.seismic.tti.operators import AdjointOperator, ForwardOperator, JacobianAdjOperator, JacobianOperator, particle_velocity_fields
...
class AnisotropicWaveSolver(object):
    ...
    def op_fwd(self, save=False):
        ...
        return ForwardOperator(self.model, save=save, geometry=self.geometry, space_order=self.space_order, kernel=self.kernel, **self._kwargs)

    @memoized_meth
    def op_adj(self):
        """Cached operator for adjoint runs"""
        return AdjointOperator(self.model, save=None, geometry=self.geometry, space_order=self.space_order, kernel=self.kernel, **self._kwargs)

    @memoized_meth
    def op_jac(self):
        """Cached operator for born runs"""
        return JacobianOperator(self.model, save=None, geometry=self.geometry, space_order=self.space_order, **self._kwargs)

    @memoized_meth
    def op_jacadj(self, save=True):
        """Cached operator for gradient runs"""
        return JacobianAdjOperator(self.model, save=save, geometry=self.geometry, space_order=self.space_order, **self._kwargs)

    def forward(self, src=None, rec=None, u=None, v=None, model=None, save=False, **kwargs):
        """
        ...
        summary = self.op_fwd(save).apply(src=src, rec=rec, u=u, v=v, dt=kwargs.pop("dt", self.dt), **kwargs)
        return rec, u, v, summary

    def adjoint(self, rec, srca=None, p=None, r=None, model=None, save=None, **kwargs):
        """
        ...
        if p is None:
            p = TimeFunction(name="p", grid=self.model.grid, staggered=stagg_p, time_order=time_order, space_order=self.space_order)
        # Create the wavefield if not provided
        if r is None:
            r = TimeFunction(name="r", grid=self.model.grid, staggered=stagg_r, time_order=time_order, space_order=self.space_order)
        ...
        if self.model.dim < 3:
            kwargs.pop("phi", None)
        # Execute operator and return wavefield and receiver data
        summary = self.op_adj().apply(srca=srca, rec=rec, p=p, r=r, dt=kwargs.pop("dt", self.dt), time_m=0 if time_order == 1 else None, **kwargs)
        return srca, p, r, summary

    def jacobian(self, dm, src=None, rec=None, u0=None, v0=None, du=None, dv=None, model=None, save=None, kernel="centered", **kwargs):
        """
        ...
        u0 = u0 or TimeFunction(name="u0", grid=self.model.grid, time_order=2, space_order=self.space_order)
        v0 = v0 or TimeFunction(name="v0", grid=self.model.grid, time_order=2, space_order=self.space_order)
        du = du or TimeFunction(name="du", grid=self.model.grid, time_order=2, space_order=self.space_order)
        dv = dv or TimeFunction(name="dv", grid=self.model.grid, time_order=2, space_order=self.space_order)
        ...
        # Execute operator and return wavefield and receiver data
        summary = self.op_jac().apply(dm=dm, u0=u0, v0=v0, du=du, dv=dv, src=src, rec=rec, dt=dt, **kwargs)
        return rec, u0, v0, du, dv, summary

    def jacobian_adjoint(self, rec, u0, v0, du=None, dv=None, dm=None, model=None, checkpointing=False, kernel="centered", **kwargs):
        """
        ...
        if checkpointing:
            u0 = TimeFunction(name="u0", grid=self.model.grid, time_order=2, space_order=self.space_order)
            v0 = TimeFunction(name="v0", grid=self.model.grid, time_order=2, space_order=self.space_order)
            cp = DevitoCheckpoint([u0, v0])
            n_checkpoints = None
            wrap_fw = CheckpointOperator(self.op_fwd(save=False), src=self.geometry.src, u=u0, v=v0, dt=dt, **kwargs)
            wrap_rev = CheckpointOperator(self.op_jacadj(save=False), u0=u0, v0=v0, du=du, dv=dv, rec=rec, dm=dm, dt=dt, **kwargs)
            ...
            summary = self.op_jacadj().apply(rec=rec, dm=dm, u0=u0, v0=v0, du=du, dv=dv, dt=dt, **kwargs)
        return dm, summary
