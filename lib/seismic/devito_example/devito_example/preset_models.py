...
from lib.seismic.devito_example.model import SeismicModel
...
def demo_model(preset, **kwargs):
    ...
    * 'marmousi3d-tti': Loads the 2D Marmousi data set from the given
    ...
    spacing = kwargs.pop("spacing", tuple([10.0 for _ in shape]))
    origin = kwargs.pop("origin", tuple([0.0 for _ in shape]))
    ...
    if preset.lower() in ["constant-elastic"]:
        ...
        return SeismicModel(space_order=space_order, vp=vp, vs=vs, b=b, origin=origin, shape=shape, dtype=dtype, spacing=spacing, nbl=nbl, **kwargs)

    if preset.lower() in ["constant-viscoelastic"]:
        # A constant single-layer model in a 2D or 3D domain
        # with velocity 2.2 km/s.
        qp = kwargs.pop("qp", 100.0)
        vs = kwargs.pop("vs", 1.2)
        qs = kwargs.pop("qs", 70.0)
        b = 1 / 2.0

        return SeismicModel(space_order=space_order, vp=vp, qp=qp, vs=vs, qs=qs, b=b, origin=origin, shape=shape, dtype=dtype, spacing=spacing, nbl=nbl, **kwargs)
    ...
    if preset.lower() in ["constant-isotropic"]:
        ...
        if density:
            kwargs["b"] = 1
        return SeismicModel(space_order=space_order, vp=vp, origin=origin, shape=shape, dtype=dtype, spacing=spacing, nbl=nbl, fs=fs, **kwargs)
    ...
    if preset.lower() in ["constant-viscoacoustic"]:
        ...
        return SeismicModel(space_order=space_order, vp=vp, qp=qp, b=b, nbl=nbl, dtype=dtype, origin=origin, shape=shape, spacing=spacing, **kwargs)
        ...
        if density:
            kwargs["b"] = 1
        return SeismicModel(space_order=space_order, vp=vp, origin=origin, shape=shape, dtype=dtype, spacing=spacing, nbl=nbl, epsilon=epsilon, delta=delta, theta=theta, phi=phi, bcs="damp", **kwargs)
        ...
        return SeismicModel(space_order=space_order, vp=v, origin=origin, shape=shape, dtype=dtype, spacing=spacing, nbl=nbl, bcs="damp", fs=fs, **kwargs)
        ...
        return SeismicModel(space_order=space_order, vp=v, vs=vs, b=b, origin=origin, shape=shape, dtype=dtype, spacing=spacing, nbl=nbl, **kwargs)

    elif preset.lower() in ["layers-viscoelastic", "twolayer-viscoelastic", "2layer-viscoelastic"]:
        ...
        qp_top = kwargs.pop("qp_top", 40.0)
        vs_top = kwargs.pop("vs_top", 0.4)
        qs_top = kwargs.pop("qs_top", 30.0)
        b_top = kwargs.pop("b_top", 1 / 1.3)
        vp_bottom = kwargs.pop("vp_bottom", 2.2)
        qp_bottom = kwargs.pop("qp_bottom", 100.0)
        vs_bottom = kwargs.pop("vs_bottom", 1.2)
        qs_bottom = kwargs.pop("qs_bottom", 70.0)
        b_bottom = kwargs.pop("b_bottom", 1 / 2.0)
        ...
        model = SeismicModel(
            space_order=space_order, vp=v, origin=origin, shape=shape, dtype=dtype, spacing=spacing, nbl=nbl, epsilon=epsilon, delta=delta, theta=theta, phi=phi, bcs="damp", fs=fs, **kwargs
        )
        ...
        if data_path is None:
            raise ValueError("Path to devitocodes/data not found! Please specify with " "'data_path=<path/to/devitocodes/data>'")
        ...
        return SeismicModel(space_order=space_order, vp=v, origin=origin, shape=v.shape, dtype=np.float32, spacing=spacing, nbl=nbl, bcs="damp", fs=fs, **kwargs)

    elif preset.lower() in ["marmousi-tti2d", "marmousi2d-tti", "marmousi-tti3d", "marmousi3d-tti"]:
        ...
        # velocity
        vp = 1e-3 * np.fromfile(os.path.join(data_path, "marmousi3D/MarmousiVP.raw"), dtype="float32", sep="")
        vp = vp.reshape(shape)

        # Epsilon, in % in file, resale between 0 and 1
        epsilon = np.fromfile(os.path.join(data_path, "marmousi3D/MarmousiEps.raw"), dtype="float32", sep="") * 1e-2
        epsilon = epsilon.reshape(shape)

        # Delta, in % in file, resale between 0 and 1
        delta = np.fromfile(os.path.join(data_path, "marmousi3D/MarmousiDelta.raw"), dtype="float32", sep="") * 1e-2
        delta = delta.reshape(shape)

        # Theta, in degrees in file, resale in radian
        theta = np.fromfile(os.path.join(data_path, "marmousi3D/MarmousiTilt.raw"), dtype="float32", sep="")
        ...
        if preset.lower() in ["marmousi-tti3d", "marmousi3d-tti"]:
            ...
            phi = np.fromfile(os.path.join(data_path, "marmousi3D/Azimuth.raw"), dtype="float32", sep="")
        ...
        return SeismicModel(
            space_order=space_order, vp=vp, origin=origin, shape=shape, dtype=np.float32, spacing=spacing, nbl=nbl, epsilon=epsilon, delta=delta, theta=theta, phi=phi, bcs="damp", **kwargs
        )
