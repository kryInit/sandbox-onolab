import sympy as sp
from devito import Eq, Operator, TensorTimeFunction, VectorTimeFunction, diag, div, grad, solve
...
def ForwardOperator(model, geometry, space_order=4, save=False, **kwargs):
    ...
    """
    l, qp, mu, qs, b, damp = model.lam, model.qp, model.mu, model.qs, model.b, model.damp

    f0 = geometry._f0
    t_s = (sp.sqrt(1.0 + 1.0 / qp**2) - 1.0 / qp) / f0
    t_ep = 1.0 / (f0**2 * t_s)
    t_es = (1.0 + f0 * qs * t_s) / (f0 * qs - f0**2 * t_s)

    # Create symbols for forward wavefield, source and receivers
    # Velocity:
    v = VectorTimeFunction(name="v", grid=model.grid, save=geometry.nt if save else None, time_order=1, space_order=space_order)
    # Stress:
    tau = TensorTimeFunction(name="t", grid=model.grid, save=geometry.nt if save else None, space_order=space_order, time_order=1)
    # Memory variable:
    r = TensorTimeFunction(name="r", grid=model.grid, save=geometry.nt if save else None, space_order=space_order, time_order=1)
    ...
    return Operator([u_v, u_r, u_t] + src_rec_expr, subs=model.spacing_map, name="Forward", **kwargs)
