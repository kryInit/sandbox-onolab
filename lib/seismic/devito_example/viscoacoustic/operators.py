import numpy as np
import sympy as sp
from devito import NODE, Eq, Function, Operator, TimeFunction, VectorTimeFunction, div, grad, solve
...
def sls_1st_order(model, geometry, p, r=None, **kwargs):
    ...
    t_s = (sp.sqrt(1.0 + 1.0 / qp**2) - 1.0 / qp) / f0
    ...
    r = r or TimeFunction(name="r", grid=model.grid, time_order=1, space_order=space_order, save=save_t, staggered=NODE)
    ...
    if forward:
        ...
        # Attenuation Memory variable
        pde_r = r.dt + (1.0 / t_s) * (r + tt * rho * div(v.forward))
        u_r = Eq(r.forward, damp * solve(pde_r, r.forward))

        # Pressure
        pde_p = m * p.dt + rho * (tt + 1.0) * div(v.forward) + r.forward - q
        ...
        pde_v = -v.dtl - grad(rho * (1.0 + tt) * p) - grad((1.0 / t_s) * rho * tt * r.backward)
...
def sls_2nd_order(model, geometry, p, r=None, **kwargs):
    ...
    # Attenuation Memory variable
    r = r or TimeFunction(name="r", grid=model.grid, time_order=2, space_order=space_order, save=save_t, staggered=NODE)

    if forward:
        # Attenuation Memory variable
        pde_r = r.dt - (tt / t_s) * rho * div(b * grad(p, shift=0.5), shift=-0.5) + (1.0 / t_s) * r
        u_r = Eq(r.forward, damp * solve(pde_r, r.forward))

        # Pressure
        pde_p = m * p.dt2 - rho * (1.0 + tt) * div(b * grad(p, shift=0.5), shift=-0.5) + r.forward - q + (1 - damp) * p.dt
        u_p = Eq(p.forward, damp * solve(pde_p, p.forward))
        ...
        # Attenuation Memory variable
        pde_r = r.dt.T + (tt / t_s) * p + (1.0 / t_s) * r
        u_r = Eq(r.backward, damp * solve(pde_r, r.backward))

        # Pressure
        pde_p = m * p.dt2 - div(b * grad((1.0 + tt) * rho * p, shift=0.5), shift=-0.5) - div(b * grad(rho * r.backward, shift=0.5), shift=-0.5) + (1 - damp) * p.dt.T
        u_p = Eq(p.backward, damp * solve(pde_p, p.backward))
...
def kv_1st_order(model, geometry, p, **kwargs):
    ...
    if forward:
        ...
        # Pressure
        pde_p = m * p.dt + rho * div(v.forward) - tau * rho * div(b * grad(p, shift=0.5), shift=-0.5)
        u_p = Eq(p.forward, damp * solve(pde_p, p.forward))
        ...
        # Pressure
        pde_p = m * p.dt.T - div(b * grad(rho * tau * p, shift=0.5), shift=-0.5) - div(b * v.backward)
        u_p = Eq(p.backward, damp * solve(pde_p, p.backward))
...
def kv_2nd_order(model, geometry, p, **kwargs):
    ...
    if forward:
        # Pressure
        pde_p = m * p.dt2 - rho * div(b * grad(p, shift=0.5), shift=-0.5) - tau * rho * div(b * grad(p.dt(x0=t0), shift=0.5), shift=-0.5) + (1 - damp) * p.dt
        ...
        # Pressure
        pde_p = m * p.dt2 - div(b * grad(rho * p, shift=0.5), shift=-0.5) - div(b * grad(rho * tau * p.dt(x0=t0).T, shift=0.5), shift=-0.5) + (1 - damp) * p.dt.T
...
def maxwell_2nd_order(model, geometry, p, **kwargs):
    ...
    if forward:
        # Pressure
        pde_p = m * p.dt2 - rho * div(b * grad(p, shift=0.5), shift=-0.5) + m * w0 / qp * p.dt(x0=t0) + (1 - damp) * p.dt
        u_p = Eq(p.forward, solve(pde_p, p.forward))
        ...
        # Pressure
        pde_p = m * p.dt2 + m * w0 / qp * p.dt(x0=t0).T + (1 - damp) * p.dt.T - div(b * grad(rho * p, shift=0.5), shift=-0.5)
        u_p = Eq(p.backward, solve(pde_p, p.backward))
...
def ForwardOperator(model, geometry, space_order=4, kernel="sls", time_order=2, save=False, **kwargs):
    """
    ...
    if time_order == 1:
        v = VectorTimeFunction(name="v", grid=model.grid, time_order=time_order, space_order=space_order, save=save_t)
        kwargs.update({"v": v})

    p = TimeFunction(name="p", grid=model.grid, time_order=time_order, space_order=space_order, save=save_t, staggered=NODE)
    ...
    return Operator(eqn + src_term + rec_term, subs=model.spacing_map, name="Forward", **kwargs)
...
def AdjointOperator(model, geometry, space_order=4, kernel="SLS", time_order=2, **kwargs):
    ...
    """
    if time_order == 1:
        va = VectorTimeFunction(name="va", grid=model.grid, time_order=time_order, space_order=space_order)
        kwargs.update({"v": va})

    pa = TimeFunction(name="pa", grid=model.grid, time_order=time_order, space_order=space_order, staggered=NODE)
    ...
    # Substitute spacing terms to reduce flops
    return Operator(eqn + src_term + rec_term, subs=model.spacing_map, name="Adjoint", **kwargs)


def GradientOperator(model, geometry, space_order=4, kernel="sls", time_order=2, save=True, **kwargs):
    """
    ...
    # Substitute spacing terms to reduce flops
    return Operator(eqn + recterm + [gradient_update], subs=model.spacing_map, name="Gradient", **kwargs)
...
def BornOperator(model, geometry, space_order=4, kernel="sls", time_order=2, **kwargs):
    ...
    P = TimeFunction(name="P", grid=model.grid, time_order=time_order, space_order=space_order, staggered=NODE)
    rp = TimeFunction(name="rp", grid=model.grid, time_order=time_order, space_order=space_order, staggered=NODE)
    rP = TimeFunction(name="rP", grid=model.grid, time_order=time_order, space_order=space_order, staggered=NODE)
    ...
    if time_order == 1:
        dv = VectorTimeFunction(name="dv", grid=model.grid, time_order=time_order, space_order=space_order)
        kwargs.update({"v": dv})
    ...
    # Substitute spacing terms to reduce flops
    return Operator(eqn1 + src_term + rec_term + eqn2, subs=model.spacing_map, name="Born", **kwargs)
