...
def iso_stencil(field, model, **kwargs):
    ...
    if len(field.data.shape) == 3:
        ...
        eq_time_update = (
            (t.spacing**2 * vp**2 / b) * ((b * field.dx(x0=x + x.spacing / 2)).dx(x0=x - x.spacing / 2) + (b * field.dy(x0=y + y.spacing / 2)).dy(x0=y - y.spacing / 2) + q)
...
def IsoAdjOperator(model, geometry, space_order=8, save=False, **kwargs):
    ...
    vp, b = model.vp, model.b
    # Create symbols for wavefield, source and receivers
    v = TimeFunction(name="v", grid=model.grid, save=geometry.nt if save else None, time_order=2, space_order=space_order)
...
def IsoJacobianFwdOperator(model, geometry, space_order=8, save=False, **kwargs):
    ...
    u0 = TimeFunction(name="u0", grid=model.grid, save=geometry.nt if save else None, time_order=2, space_order=space_order)
    ...
    return Operator(eqn1 + src_term + eqn2 + rec_term, subs=spacing_map, name="IsoJacobianFwdOperator", **kwargs)
...
def IsoJacobianAdjOperator(model, geometry, space_order=8, save=True, **kwargs):
    ...
    return Operator([dm_update] + eqn + rec_term, subs=spacing_map, name="IsoJacobianAdjOperator", **kwargs)
