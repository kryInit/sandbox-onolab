...
def acoustic_setup(shape=(50, 50, 50), spacing=(15.0, 15.0, 15.0), tn=500.0, kernel="OT2", space_order=4, nbl=10, preset="layers-isotropic", fs=False, **kwargs):
    model = demo_model(preset, space_order=space_order, shape=shape, nbl=nbl, dtype=kwargs.pop("dtype", np.float32), spacing=spacing, fs=fs, **kwargs)
...
def run(
    shape=(50, 50, 50), spacing=(20.0, 20.0, 20.0), tn=1000.0, space_order=4, kernel="OT2", nbl=40, full_run=False, fs=False, autotune=False, preset="layers-isotropic", checkpointing=False, **kwargs
):

    solver = acoustic_setup(shape=shape, spacing=spacing, nbl=nbl, tn=tn, space_order=space_order, kernel=kernel, fs=fs, preset=preset, **kwargs)
...
def test_isoacoustic_stability(shape, k, interp):
    ...
    _, _, _, [rec, _] = run(shape=shape, spacing=spacing, tn=20000.0, nbl=0, kernel=k, interpolation=interp)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--fs", dest="fs", default=False, action="store_true", help="Whether or not to use a freesurface")
    parser.add_argument("-k", dest="kernel", default="OT2", choices=["OT2", "OT4"], help="Choice of finite-difference kernel")
