...
\newcommand{\projBox}[1]{ P_{[l,u]^N} \left( #1 \right) }
\newcommand{\projBoxSolution}{ \projBox{\vecx} = \text{min}( \text{max} (\vecx, l), u) }
\newcommand{\projLOneTwoBall}[1]{P_{ \{ \LOneTwoNorm{ \cdot } \le \alpha \} }\left(#1\right)}
...
\newcommand{\projLOneBallSolutionWhere}{
    \begin{aligned}
        ...
        & \beta'             = \text{max} \left\{ \frac 1 i \left(\left(\sum_{j=1}^i \bm y_j\right) - \alpha\right) \middle| \ i \in \mathbb{N}, 1 \le i \le N \right\}, \\
...
\newcommand{\FWIObjectiveWithTVConstraintWithIndicatorFunction}{ E(\velModel) + \indicatorFunction{\LOneTwoNorm{\cdot} \le \alpha}{\diffOperator \velModel} + \indicatorFunction{[l,u]^N}{\velModel} }
