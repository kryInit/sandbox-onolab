...
\newcommand{\projLOneTwoBallSolution}{
    \begin{aligned}[t]
        & \projLOneTwoBall{\vecx} = [ \bm p_1^T, \ldots, \bm p_N^T ]^T \\
        & \text{where} \\
        & \ \ \ \ \begin{aligned}
            & \bm p_i = \begin{cases}
...
\newcommand{\projLOneBallSolution}{
    \begin{aligned}[t]
        & \projLOneBall{\vecx} = \text{SoftThrethold}(\vecx, \beta)  \\
        & \text{where} \\
        & \ \ \ \ \begin{aligned}
...
\newcommand{\FWIObjectiveWithTVConstraint}{ E(\velModel) \ \ \ \text{s.t.} \ \ \TV{\velModel} \le \alpha \ , \ \velModel_{i,j} \in [a,b]^N }
...
\newcommand{\FWIWithPDS}{
    \begin{aligned}
        ...
        & \vecx^{(k+1)}                 = \widetilde{\vecy}^{(k+1)} - \gamma_2 \projLOneTwoBall{\frac 1 {\gamma_2} {\widetilde{\bm y}^{(k+1)}}}
    \end{aligned}
}
