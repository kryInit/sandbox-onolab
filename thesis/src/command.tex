...
\newcommand{\conjugateFunctionDefinition}{f^*(\vecy) \coloneq \sup_{\vecx} \left\{ \vecx^T \vecy - f(\vecx) \right\}}
...
\newcommand{\proximityOperatorForBoxConstraint}{ P_{[a,b]^N}({\vecx}) = \text{min}( \text{max} (\vecx, a), b) }
\newcommand{\projLOneTwoBall}{
    \begin{aligned}
        & P_{ \{ \bm a \mid \LOneTwoNorm{ \bm a } \le \alpha \} }({\vecx}) = [ \bm p_1^T, \ldots, \bm p_N^T ]^T \\
...
\newcommand{\projLOneBall}{
    \begin{aligned}
        & P_{ \{ \bm a \mid \LOneNorm{ \bm a } \le \alpha \} }({\vecx}) = \text{SoftThrethold}(\vecx, \beta)  \\
        & \ \ \ \ \text{with} \ \  \begin{aligned}
...
% FWI objective
\newcommand{\FWIObjective}{ \frac {1} {2} \LTwoNorm { \seismicData_{\mathrm{obs}} - \seismicData_{\mathrm{cal}(\velModel)} }^2 }
\newcommand{\FWIObjectiveWithTVConstraint}{ \FWIObjective \ \ \ \text{s.t.} \ \ \TV{\velModel} \le \alpha \ , \ \velModel_{i,j} \in [a,b] }
\newcommand{\FWIObjectiveWithTVConstraintWithIndicatorFunction}{ \FWIObjective + \indicatorFunction{\LOneTwoNorm{\cdot} \le \alpha}{\diffOperator \velModel} + \indicatorFunction{[a,b]}{\velModel} }
\newcommand{\FWIGradient}{  }
