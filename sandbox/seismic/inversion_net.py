...
def predict(seismic_data_batch, dataset_config: dict):
    # modelのrestore
    model = network.model_dict["InversionNet"](upsample_mode=None, sample_spatial=1.0, sample_temporal=1, norm="bn").to("cpu")
...
def main():
    ...
    transform_valid_label = Compose([T.MinMaxNormalize(dataset_config["label_min"], dataset_config["label_max"])])
    true_data = transform_valid_label(raw_true_data.copy())

    restore = lambda x: ((x / 2 + 0.5) * (dataset_config["label_max"] - dataset_config["label_min"]) + dataset_config["label_min"]) / 1000.0

    pred = predict(train_data, dataset_config=dataset_config)
    np.save(datasets_root_path.joinpath("open-fwi/tmp/pred.npy"), pred)

    show_velocity_model(raw_true_data[0][0] / 1000.0, title="True Velocity Model", vmin=1.5, vmax=4.5)
