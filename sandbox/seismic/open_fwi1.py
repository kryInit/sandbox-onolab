# open-fwiを調べるために使った検証用コード, 多分プリミティブなシミュレーションで自分の理解が間違っていないかを確認したが、dampingが雑でノイズが入るのであんまりうまくいかないなぁとなって使わなくなった
# 結局devitoが使えたのでそっちを使っているはず(open_fwi.py
...
def generate_input_source_devito_function(
    ...
    src = SparseTimeFunction(name="src", grid=grid, time_order=2, space_order=1, nt=simulation_times + 1, npoint=1)
...
def visualize(params: Params, grid: Grid, source_waveform: SparseTimeFunction, field_logs: FieldLog):
    ...
    plt.plot(source_waveform.data), plt.xlabel("time [ms]"), plt.ylabel("Amplitude"), plt.title("source waveform"), plt.show()
...
def main():
    ...
    params = Params(
        ...
        source_cell_pos_in_field=Vec2D(35, 0),
    )
