...
def show_images():
    # n = 128
    seismic_data_path = datasets_root_path.joinpath("thebe/faulttest1.npz")
    fault_data_path = datasets_root_path.joinpath("thebe/faulttest1.npy")
    seismic_data = np.load(seismic_data_path)["arr_0"]
    fault_data = np.load(fault_data_path)
    print(np.all(seismic_data == fault_data))
    ...
    def show_by_plt():
        ...
        ax.plot_surface(
            ...
            rstride=1,
            cstride=1,
            facecolors=plt.cm.gray(seismic_data[0, :, :]),
            cmap="gray",
            shade=False,
            zorder=-1,
        )
